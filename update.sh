...
grep -h -E '^\s+[A-Z0-9]+(_[A-Z0-9])+' /tmp/dv/providers/**/*.toml | sort -b -u -k1,1 | sed -e 's/=/ = /' | awk '{split($0,a,"="); printf "  %s:\n    description: %s\n    required: false\n", $1, a[2]}' > /tmp/dv/settings.yml

# Create a list of environment variables for DNS providers
grep -h -E '^\s+[A-Z0-9]+(_[A-Z0-9])+' /tmp/dv/providers/**/*.toml | sort -b -u -k1,1 | sed -e 's/=/ = /' | awk '{printf "    %s: ${{ inputs.%s }}\n", $1, $1}' > /tmp/dv/env.yml
